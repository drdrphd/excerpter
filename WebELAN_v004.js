...
//
// exportExcerptForViewer(Array[String], Array[String])
//
// Takes lists of (top)tier names and annotation IDs to excerpt
//
// Exports Tiers & Annotations to JS Arrays
// const annotations =
// [
...
// Creates a blob, initiates download of the blob
//
WebELAN.prototype.exportExcerptForViewer = function(tier_names, annotation_ids) {
	var selected_tiers = {};
	for (var i = 0; i < this.tiers.length; i++) {
		var tier = this.tiers[i];
		var id = tier.getAttribute("TIER_ID");
		if (tier_names.includes(id)) {
			var tier_obj = {};
			...
			selected_tiers[id] = tier_obj;
		}
	}
	
	//we only receive the top-tier annotation ids
	//need to go through tiers hierarchically
	//start at top, append annotations from ids
	//then go through next tiers --
	//if parent-ref is in annotation_ids, append annotation
	var selected_annotations = {};
	var h_tiers = this.sortTiersHierarchical(this.tiers);
	
	for (var i = 0; i < h_tiers.length; i++) {
		var tier = h_tiers[i];
		var tier_annotations = [];
		if (tier_names.includes(tier.getAttribute("TIER_ID"))) {
			var annotations = this.annotations(tier);
			for (var j = 0; j < annotations.length; j++) {
				var annotation_values = [];
				var a = annotations[j];
				...
				if (annotation_ids.includes(id) || annotation_ids.includes(p)) {
					annotation_values.push(id);
					annotation_values.push(p);
					annotation_values.push(t);
					annotation_values.push(this.value(a));
			...
			selected_annotations[tier.getAttribute("TIER_ID")] = tier_annotations;
		}
	}
	
	return [selected_tiers, selected_annotations, this.filename];
}
